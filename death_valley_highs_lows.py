...
with open(filename) as f:
    ...
    for row in reader:  # Об'єкт reader продовжує роботу там, де зупинився останній раз
        ...
        try:
            high = int(row[4])  # Витягаємо данні з індексом [5] -> TMAX == максимальна температура
            low = int(row[5])  # Витягаємо данні з індексом [6] -> TMIN == максимальна температура
        except ValueError:
...
plt.title('Максимальні та мінімальні добові температури, 2018 рік\nДолина Смерті, Каліфорнія', fontsize=20)  # Задаємо назву
